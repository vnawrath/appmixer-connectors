...
module.exports = {
    ...
    async start(context) {
        ...
        const chatId = context.properties.chatId;
        
        if (!botToken) {
            throw new Error('Telegram bot token is required. Please configure authentication.');
        }

        if (!chatId) {
            throw new Error('Chat ID is required. Please specify the chat ID to monitor.');
        }
        ...
        try {
            ...
            if (error.message.includes('ENOTFOUND') || error.message.includes('401')) {
    ...
    async receive(context) {
        if (context.messages.webhook) {
            ...
            if (update.message) {
                ...
                // Filter by chat ID
                if (context.properties.chatId && message.chat.id.toString() !== context.properties.chatId.toString()) {
                    return; // Ignore messages from other chats
                }
                
                // Filter bot messages if requested
                if (context.properties.ignoreBotMessages && message.from && message.from.is_bot) {
                    return; // Ignore bot messages
                }
                
                // Filter by message type if specified
                if (context.properties.messageType && context.properties.messageType !== 'all') {
